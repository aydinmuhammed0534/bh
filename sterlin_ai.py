...
def forecast_currency(currency_pair):
    ...
    if data.empty:
        logging.error(f"No data found for {currency_pair}.")
        return 0
    ...
    if currency_pair == 'USDTRY=X':
        model = Prophet(seasonality_mode='additive', yearly_seasonality=True, weekly_seasonality=True, changepoint_prior_scale=0.05)
    ...
    logging.info(f"Forecast for {currency_pair}: {forecast[['ds', 'yhat']].tail()}")
